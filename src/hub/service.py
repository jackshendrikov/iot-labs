"""Сервіс накопичення та пакетного збереження дорожніх даних."""
...
class HubService:
    ...
    async def start(self, *, enable_mqtt: bool = True) -> None:
        ...
        logger.info(
            f"Hub запущено без MQTT-транспорту, batch_size={self._batch_size}, flush_interval={self._flush_interval_seconds}s"
        )
    ...
    async def ingest(self, data: ProcessedAgentData) -> None:
        """Приймає вже валідовані дані з будь-якого транспортного шару."""
    ...
    def _on_connect(
        ...
        """Підписує MQTT-клієнт на топік Hub після успішного connect."""
        if reason_code == 0:
    ...
    async def _periodic_flush_loop(self) -> None:
        """Періодично ініціює примусовий flush накопичених даних."""
        while True:
            try:
    ...
    async def _persist_backlog(self, batch: list[ProcessedAgentData]) -> None:
        """Складає невдалий batch у Redis для повторної відправки."""
        assert self._redis is not None
        redis_client = cast(aioredis.Redis, self._redis)
        if not batch:
            return
